...
\providecommand{\untertitel}{Vorlesung Musikdatenanalyse\\WiSe\,25/26}
\providecommand{\ausgabedatum}{29.01.2026}
...
\begin{aufgabe}{Lindenmayer-System}
	...
	\end{teilaufgabe}
	\begin{tipp}
		Arbeiten Sie mit Strings. {\emph{Achtung}}: String-Funktionen wie \texttt{replace} oder \texttt{replaceAll} sind hier meist \textbf{nicht} hilfreich!
	\end{tipp}
...
\begin{aufgabe}{Expertensystem}
	...
	\begin{loesung}
		...
		Ein Beispiel: $F(\{c,d,e\}) = f(c) + f(d) + f(e) = 1+(-1)+1 = 1$\\
		Daraus folgt: $A(\{c,d,e\}) = C-Dur-Akkord$
