...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Studium Jazzgitarre an der Folkwang UdK Essen
		\item Lehre
		\begin{itemize}
			\item Fachprojekt \emph{Digital Entertainment Technologies}
			\item Fachprojekt \emph{Musikinformatik}
			\item Übung zu \emph{Praktische Optimierung}
		\end{itemize}
		\item Forschung zu
		\begin{itemize}
			\item Computer Music
			\item Generative AI (Evolutionary Algorithms \& Reinforcement Learning)
			\item AI \& Procedural Content Generation in Games
		\end{itemize}
	\end{itemize}
\end{frame}
...
%vgl. 24.2.2 Why Automatic Composition?
	...
	\begin{itemize}
		...
		\medskip
		\item[\Conclude] \underline{Heute}: wissenschaftliche (nüchterne) Betrachtung unterschiedlicher Verfahren
	\end{itemize}
...
\begin{frame}{Geschichte ohne Computer}{Musikalisches Würfelspiel}
	...
	\hfill \footnotesize Digitalisierte Version: \href{https://www.buschs.de/Mozart/}{\texttt{https://www.buschs.de/Mozart/}}
...
\begin{frame}{Geschichte mit Computer}{Iannis Xenakis (1922--2001) -- \emph{Stochastische} Musik}
	...
	\enquote{Achorripsis} (1957) basiert auf der Poisson-Verteilung $P_\lambda(k) = \frac{\lambda^k}{k!}e^{-\lambda}$
...
\begin{frame}[t]{Geschichte mit Computer}{Illiac Suite (String Quartet No. 4)}
	
	\drawat{11.5cm,2.5cm}{\includegraphics[width=4cm]{img/hiller.jpg}}
	\drawat{11.5cm,6cm}{\includegraphics[width=4cm]{img/Hiller-exp1-text.png}}
	
	\begin{itemize}
		\item Gilt als \enquote{erster, ernsthafter Versuch} computer-gestützter Komposition
		\item 4 Sätze:
		\begin{enumerate}
...
\begin{frame}{II. \dimTwoTitle}{Gary Lee Nelson -- \enquote{Fractal Mountains} (1988--89)}
	Beispiel zur Erzeugung von Tonhöhen mit Fraktalen:
	
	\begin{center}
	\includegraphics[width=.8\textwidth]{img/Fractal_Mointain.png}
	\end{center}
	
	\conclude~96 Tonschritte pro Oktave\\
	Weitere Fraktalfunktionen für Tonlänge und Lautstärke
...
\begin{frame}{II. \dimTwoTitle}{Markov-Kette (Beispiel 2)}
	...
	\medskip
	\hspace{4.5cm}\underline{Erweiterung}, z.B. auf relative Intervalle, Rhythmus, ...\\
	\hspace{4.5cm}\underline{Abhängigkeiten erhöhen}: Markow-Kette n-ter Ordnung\\
	\smallskip
	\hspace{4.5cm}\underline{Lernverfahren}: \emph{Hidden Markov Model}
	
\end{frame}

\begin{frame}{II. \dimTwoTitle}{Markov-Kette (Hörbeispiele)}
	
	Beethoven \enquote{Für Elise} \audioplayer{audio/elise_detailed.mp3}\\
	\bigskip
	Markov-Kette~10.~Ordnung \audioplayer{audio/elise_detailed_markov_10.mp3}\\
	\smallskip
	Markov-Kette~~5.~Ordnung \audioplayer{audio/elise_detailed_markov_5.mp3}\\
	\smallskip
	Markov-Kette~~3.~Ordnung \audioplayer{audio/elise_detailed_markov_3.mp3}\\
	\smallskip
	Markov-Kette~~1.~Ordnung \audioplayer{audio/elise_detailed_markov_1.mp3}\\
	
	\vfill\footnotesize
	Jupyter-Notebook zur Reproduktion:\\
...
\begin{frame}[t]{III. \dimThreeTitle}{Lernverfahren (datenbasiert)}
	\parbox[c]{.49\textwidth}{
		...
		\[ o_j = \varphi ( \theta_j + \sum_{i=1}^n w_{ij} x_{ij} ) \]
	}
	\parbox[c]{.49\textwidth}{
		\vspace{.5cm}
		\underline{Neuronales Netz}
		\smallskip
		\\Mehrlagiges Perzeptron (MLP)\\
		\includegraphics[width=0.49\textwidth]{img/MLP-Theorie.png}
		
		\raggedleft
		Lernt Merkmale \emph{quasi} selbstständig\\
		\smallskip
		\conclude~\emph{Datenart} nahezu irrelevant
	}
\end{frame}
...
\begin{frame}{V. \dimFiveTitle}{Evolutionärer Algorithmus (EA)}
	...
	\hfill Conference on  \textbf{AI in Music, Sound, Art and Design} (EvoMUSART)\\
	\small\hfill\url{https://www.evostar.org/2025/evomusart/}
\end{frame}
...
\begin{frame}{V. \dimFiveTitle}{EAR Drummer (2017)}
	...
	\begin{columns}
		\begin{column}{.5\textwidth}
			...
			Fitness ist Grad der musikalischen Passung:\\
			\hspace{2.5cm} \fbox{\emph{human solo} $\overset{?}{=}$ \emph{drum pattern}}
		\end{column}
		\begin{column}{.6\textwidth}
			\underline{Umsetzung}: Heuristische Regeln auf Notenstatistiken:
			...
			\begin{itemize}\scriptsize\setlength{\itemsep}{0pt}\setlength{\itemindent}{.7cm}
				\item[Keep:] Erhalte Ausgangspattern
				\item[Reaction:] Bewerte \emph{drum pattern} nach einer heuristischen Regel unter Bezug auf Muster in der Notenstatistik
				\item[Random:] Rauschen erzeugt mehr zufälliges Verhalten
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}[t]{VI. \dimSixTitle}{Human-In-The-Loop: GenJam (1994)}
	...
	Al Biles modellierte GenJam \citep{GenJam1994} als künstlichen \textbf{Musikschüler},\\
...
\begin{frame}{VI. \dimSixTitle}{iMuse in \emph{Monkey Island 2} (1991)}
	...
	\hfill\includegraphics[height=.7\textheight]{img/woodtick_score.jpg}
	\vspace{-2cm}
	
	Alle Transitionen sind manuell komponiert\\
	\conclude~Nur bei \emph{Blockbuster}-Produktionen realistisch\\
...
\hspace{2.2cm}\url{https://aimusiccreativity.org/conferences/}
...
\begin{frame}[t]{VI. \dimSixTitle}{MuseBots}
	\centering
	\begin{columns}
		\begin{column}{0.1\textwidth}
		\end{column}
		\begin{column}{0.45\textwidth}
			\underline{Idee}: einheitliches Protokoll zur Zusammenschaltung verschiedener musikalischer \emph{Agenten}
			\bigskip
...
\begin{frame}{Subjektive Evaluation}
	...
	\smallskip
	~~~\emph{Kategorische Ablehnung} (negative bias) gegenüber KI-Musik \citep{AIComposerBias,AIPerformerBias}\\
	\smallskip
	\hfill\conclude~kann bereits bei Ankündigung/Bewerbung einer Studie problematisch sein
\end{frame}
...
\begin{frame}{Aktuell im Fokus}{\enquote{Was heißt hier Intelligenz?}}
	...
	\begin{itemize}
		...
		\item Diffusion Models\\
			\qquad\conclude~Entrauschen eines zufälligen Initials
		\item Transformer-Modelle\\
		\qquad\conclude~eigenes Thema für ein ganzes Seminar...
	\end{itemize}

	\hfill \footnotesize $^*$etwa $10^6$--$10^9$ Parameter
\end{frame}
...
\begin{frame}{Variational Autoencoder}{(VAE)}
	\begin{columns}
		\begin{column}{.5\textwidth}
			...
			\raggedleft
			Jeder bekannte Datenpunkt = ein Punkt im \emph{latenten} Raum\\
			\vspace{-.2cm}
		...
		\begin{column}{.5\textwidth}
			...
			\raggedleft
			Jeder bekannte Datenpunkt = Verteilung im latenten Raum\\
			\raggedright
			\[z \sim \mathcal{N}(\mu(x), \sigma^2(x))\]
			
			Backpropagation wird ermöglicht durch \enquote{Reparametrization \underline{trick}}:\\
			\[z = \mu(x) + \sigma(x) \cdot \epsilon\]
			
			$\mu$ und $\sigma$ werden gelernt\\
			$\epsilon$ wird beim Training variiert
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Generative Adversarial Network}{(GAN)}
	\centering
	\includegraphics[width=0.7\textwidth]{img/GAN_template.png}
	
	\vfill
	\hfill\scriptsize Bildquelle: \url{https://doi.org/10.23889/ijpds.v8i1.2158}
\end{frame}
...
\begin{frame}[t]{Diffusion Models}
	...
	Idee: Schrittweises \emph{Entrauschen} einer Zufallseingabe
	\medskip
	
	$q$: Verrauschen des Datensatzes (Umkehrung unlösbar)\\
	$p_\theta$: Geschätzte (gelernte) Entrauschungsfunktion (eng. \emph{denoising})
	\medskip
	
	Besonders interessant: Teilweise Wiederherstellung \conclude~\enquote{Inpainting}
	
	\drawat{11.5cm,5.5cm}{\includegraphics[height=2.8cm]{img/librosa-feature-melspectrogram-1.png}}
\end{frame}
...
\begin{frame}{Deep Learning-Systeme}
		...
		\begin{itemize}
			\item datenbasiert
			\item stochastisch\\
			~~~\conclude~{\small(bei fester Eingabe deterministisch)}
...
\begin{frame}{Die Zukunft?}{Prompt-Systeme}
	...
	\medskip
	Prompt-basierte Modelle für Musik existieren, z.B. MusicLM \& StableAudio.\\ 
	
	\bigskip
	\hfill \underline{Spannend bleibt}: \enquote{Was werden diese Systeme nicht lernen können?}\hspace{-.5cm}	
\end{frame}

\begin{frame}{Urheberrecht?}
	\vspace{-.2cm}
	\tiny
	\begin{columns}
...
\begin{frame}[t]{Originalität wird zum Problem}{Reinforcement Learning with Human Feedback}
	\begin{center}
		\includegraphics[height=.8\textheight]{img/RLHF-icon.png}
	\end{center}
	\vspace{-3cm}
	\hfill\conclude~Verhindert \emph{model collapse} \cite{?}
	
	\bigskip
	\hfill(Idee ähnlich zu \emph{GenJam})
\end{frame}
...
\begin{frame}{Held Horizon (2025)}{AI–human co-creation experiment on composing for choir}
	...
	\vfill
	Erläuterungen und Transkript des \enquote{Kompositionsdialogs}:\\
